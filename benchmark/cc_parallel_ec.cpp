...
#pragma omp parallel for
        for (int i = 0; i < THREAD_NUM; i++)
        {
            ...
            EdgeCursor* edge_cur = graph->get_edge_iter();
            edge_cur->set_key_range(g.get_edge_range(i));
           // std::cout<<g.get_edge_range(i).start.src_id << " " << g.get_edge_range(i).start.dst_id <<"\n";
            edge u;
            edge_cur->next(&u);
            while (u.src_id != UINT32_MAX)
            {
                //std::cout<<u.src_id << " " << u.dst_id <<"\n";
                node_id_t comp_u = comp[u.src_id];
                node_id_t comp_v = comp[u.dst_id];
                edge_cur->next(&u);
